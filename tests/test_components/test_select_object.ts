...
import { emitted, expect_html_element_has_value, set_data } from '@/tests/utils';
...
describe('select-object tests', () => {
    ...
    test('Items list changed', async () => {
        ...
        expect_html_element_has_value(wrapper.find('select'), new_things[1].id.toString());
    });
...
test('v-model usage', async () => {
    ...
    let select_object = wrapper.findComponent(SelectObject);
    expect_html_element_has_value(select_object.find('select'), things[1].id.toString());

    await set_data(wrapper, {selected: things[2]}, false);
    expect_html_element_has_value(select_object.find('select'), things[2].id.toString());
