...
beforeEach(() => {
    const project = data_ut.make_project(data_ut.make_course().pk);
...
async function make_wrapper(user_roles: Partial<ag_cli.UserRoles> = {}) {
    const wrapper = managed_mount(AGTestCommandResultDetail, {
        propsData: {
...
describe('Correctness feedback tests', () => {
    ...
    describe('Return code correctness tests', () => {
        ...
        test('Return code correct, timeout fdbk hidden', async () => {
            ...
            const wrapper = await make_wrapper();
            const section_wrapper = wrapper.findComponent({ref: 'return_code_correctness'});
    ...
    describe('Output correctness tests', () => {
        ...
        test('Stdout correct', async () => {
            ...
            const wrapper = await make_wrapper();
            const section_wrapper = wrapper.findComponent({ref: 'stdout_correctness'});
            expect(section_wrapper.find('.correct-icon').exists()).toBe(true);
            expect(section_wrapper.find('.incorrect-icon').exists()).toBe(false);
        });
...
describe('Diff tests', () => {
    ...
    test('Stdout diff available', async () => {
        ...
        const diff_wrapper =  <Wrapper<Diff>> wrapper.findComponent({ref: 'stdout_diff'});
    ...
    test('Stderr diff available', async () => {
        ...
        expect(wrapper.findComponent({ref: 'diffs'}).exists()).toBe(true);
        const diff_wrapper =  <Wrapper<Diff>> wrapper.findComponent({ref: 'stderr_diff'});
        expect(await diff_wrapper.vm.diff_contents).toEqual(diff);
        expect(diff_wrapper.vm.progress).not.toBeNull();
    ...
    test('Both diffs available', async () => {
        ...
        const stdout_diff_wrapper =  <Wrapper<Diff>> wrapper.findComponent({ref: 'stdout_diff'});
        expect(await stdout_diff_wrapper.vm.diff_contents).toEqual(stdout_diff);
        expect(stdout_diff_wrapper.vm.progress).not.toBeNull();
        const stderr_diff_wrapper =  <Wrapper<Diff>> wrapper.findComponent({ref: 'stderr_diff'});
...
describe('Actual output tests', () => {
    ...
    test('Actual stdout available', async () => {
        const stdout = 'this is some very stdout';
        output_size_resolves(submission.pk, ag_test_command_result.pk,
        ...
        const stdout_wrapper = <Wrapper<ViewFile>> wrapper.findComponent({ref: 'stdout'});
    ...
    test('Actual stderr available', async () => {
        ...
        const stderr_wrapper = <Wrapper<ViewFile>> wrapper.findComponent({ref: 'stderr'});
    ...
    test('Actual stdout and stderr available', async () => {
        const stdout = 'moar stdout';
        const stderr = 'such stderr';
        output_size_resolves(submission.pk, ag_test_command_result.pk,
...
describe('Command Descriptions', () => {
     beforeEach(() => {
        const test_command = data_ut.make_ag_test_command(
    ...
    test('Staff description shown to staff', async () => {
        const wrapper = await make_wrapper({is_staff: true});

        expect(await wait_until(wrapper, w => w.vm!.d_staff_description !== null)).toBe(true);
        const staff_description = wrapper.findComponent({ref: 'staff_description'});
    ...
    test('Staff description not shown to student', async () => {
        const wrapper = await make_wrapper({is_staff: false, is_student: true});
    ...
    test('Student description visible', async () => {
        ...
        const wrapper = await make_wrapper();
        const student_description = wrapper.findComponent({ref: 'student_description'});
    ...
    test('Student on-fail description visible', async () => {
        ...
        const wrapper = await make_wrapper();
        const student_description = wrapper.findComponent({ref: 'student_on_fail_description'});
